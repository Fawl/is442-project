...
export default function DashboardPage() {
  ...
  const updateMetrics = async () => {
    ...
    const filteredTickets: EventTickets = tickets.filter(ticket => {
    ...
    const totalRevenue = Object.values(filteredTickets).flat().reduce((total, ticket) => total + ticket.price, 0)
    const ticketsSold = Object.values(filteredTickets).reduce((total, tickets) => total + tickets.length, 0)
    ...
    Object.values(filteredTickets).flat().forEach(ticket => {
      const purchaseDate = new Date(ticket.purchaseTime)
  ...
  const selectEvent = (eventName: any) => {
    setTicketEvents((prevEvents: { eventName: any; selected: any; }[]) => prevEvents.map((event: { eventName: any; selected: any; }) => {
  ...
  return(<div className="grid grid-cols-3 gap-4">
    ...
    <div className="grid grid-cols-subgrid gap-4 col-span-3">
      ...
      <div className="col-start-3 col-span-1">  
        <DownloadButton summaryMetrics={summaryMetrics} revenueByMonth={revenueByMonth} eventList={ticketEvents}/>
      </div>
    </div>
    ...
    <div className="grid grid-cols-subgrid gap-4 col-span-3">
      ...
      <SummaryMetrics metricName={"Average Ticket Price"} metricValue={summaryMetrics.totalRevenue} />
    </div>
